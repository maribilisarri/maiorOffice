...
def duties():
    ...
    for file in os.listdir(combined_blocks_dir):
        ...
        # print(f" ✔ Ολοκληρώθηκε: {filename} | Sheet: {sheet_name}\n")
        print(f" ✔ Ολοκληρώθηκε: ")


        # #----------------------------αρχικο τελικο node με depot
        #
...
def add_start_end_depot_info():
    input_folder = os.path.join('duties-output', 'short')
    output_folder = os.path.join('duties-output', 'startendDepot')
    ...
    for file in os.listdir(input_folder):
        if file.endswith('.csv'):
            ...
            # Κάνε group by REFERREDVB_COMPANYCODE
            grouped = df.groupby('REFERREDVB_COMPANYCODE')
            updated = []

            for refvb, group in grouped:
                group = group.copy()
                group.sort_values(by='START_TIME', inplace=True)

                if not group.empty:
                    first_idx = group.index[0]
                    last_idx = group.index[-1]

                    group.at[first_idx, 'STARTNODE_COMPANYCODE'] = group.at[first_idx, 'PROJECT_NAME']
                    group.at[last_idx, 'ENDNODE_COMPANYCODE'] = group.at[last_idx, 'PROJECT_NAME']

                updated.append(group)

            final_df = pd.concat(updated).reset_index(drop=True)

            output_path = os.path.join(output_folder, file)
            final_df.to_csv(output_path, sep=';', index=False, encoding='utf-8')
            print(f"✅ Αποθηκεύτηκε: {output_path}")

def add_start_time(short_dir, deadhead_file, output_dir):
    os.makedirs(output_dir, exist_ok=True)

    deadhead_df = pd.read_csv(deadhead_file, sep=';', dtype=str)
    deadhead_df['RUNNINGTIME'] = pd.to_timedelta(deadhead_df['RUNNINGTIME'])
    ...
    for filename in os.listdir(short_dir):
        ...
        for vb_code, group in df.groupby('REFERREDVB_COMPANYCODE'):
            if group.empty:
                continue
            ...
            project_name = group_sorted.loc[first_idx, 'PROJECT_NAME']
            start_node = group_sorted.loc[first_idx, 'STARTNODE_COMPANYCODE']
            end_node = group_sorted.loc[last_idx, 'ENDNODE_COMPANYCODE']

            # Lookup for start deadhead time
            match_start = deadhead_df[
                (deadhead_df['ORIGINNODE'] == project_name) &
                (deadhead_df['DESTINATIONNODE'] == start_node)
                ]
            if not match_start.empty:
                deadhead_time_start = match_start['RUNNINGTIME'].iloc[0]
                delta_start = pd.to_timedelta(deadhead_time_start)
                original_time = df.at[first_idx, 'START_TIME']
                if pd.notna(original_time):
                    new_start = original_time - delta_start
                    updated.at[first_idx, 'START_TIME'] = new_start.strftime("%H:%M:%S")
                updated.at[first_idx, 'STARTNODE_COMPANYCODE'] = project_name

            # Lookup for end deadhead time
            match_end = deadhead_df[
                (deadhead_df['ORIGINNODE'] == end_node) &
                (deadhead_df['DESTINATIONNODE'] == project_name)
                ]
            if not match_end.empty:
                deadhead_time_end = match_end['RUNNINGTIME'].iloc[0]
                delta_end = pd.to_timedelta(deadhead_time_end)
                original_end = df.at[last_idx, 'END_TIME']
                if pd.notna(original_end):
                    new_end = original_end + delta_end
                    updated.at[last_idx, 'END_TIME'] = new_end.strftime("%H:%M:%S")

                updated.at[last_idx, 'ENDNODE_COMPANYCODE'] = project_name

        # Αποθήκευση νέου αρχείου
        out_path = os.path.join(output_dir, filename)
        updated.to_csv(out_path, sep=';', index=False, encoding='utf-8')
        print(f"✅ Αποθηκεύτηκε με deadhead ώρα στο: {out_path}")
    # deadhead_file = 'static/Deadhead_links_and_running_ times.csv'
    # short_duties_dir = 'duties-output/short'
    # output_duties_time_dir ='duties-output/time'
    # os.makedirs(output_duties_time_dir, exist_ok=True)
    #
    # deadhead_df = pd.read_csv(deadhead_file, sep=';', dtype=str)
    # deadhead_df['RUNNINGTIME'] = pd.to_timedelta(deadhead_df['RUNNINGTIME'])
    #
    # results = []
    #
    # # Process each short duty file
    # for file in os.listdir(short_duties_dir):
    #     if file.endswith('.csv'):
    #         path = os.path.join(short_duties_dir, file)
    #         df = pd.read_csv(path, sep=';', dtype=str)
    #         df['START_TIME'] = pd.to_datetime(df['START_TIME'], format="%H:%M:%S", errors='coerce')
    #
    #         updated_rows = []
    #         for refvb, group in df.groupby('REFERREDVB_COMPANYCODE'):
    #             group = group.copy()
    #             group.sort_values(by='START_TIME', inplace=True)
    #
    #             if not group.empty:
    #                 first = group.iloc[0]
    #                 origin = first['PROJECT_NAME']
    #                 dest = first['STARTNODE_COMPANYCODE']
    #
    #                 # Find matching link
    #                 match = deadhead_df[
    #                     (deadhead_df['ORIGINNODE'] == origin) &
    #                     (deadhead_df['DESTINATIONNODE'] == dest)
    #                     ]
    #
    #                 if not match.empty:
    #                     runtime = match.iloc[0]['RUNNINGTIME']
    #                     try:
    #                         adjusted_time = first['START_TIME'] - runtime
    #                         group.iloc[0, group.columns.get_loc('START_TIME')] = pd.Timestamp(adjusted_time)
    #                     except Exception as e:
    #                         pass  # Skip if conversion fails
    #
    #             updated_rows.append(group)
    #
    #         updated_df = pd.concat(updated_rows).reset_index(drop=True)
    #         results.append((file, updated_df))
    #         output_path = os.path.join(output_duties_time_dir, file)
    #         updated_df.to_csv(output_path, sep=';', index=False, encoding='utf-8')
    #def adjust_start_end_with_deadhead(short_dir, deadhead_file, output_dir):
...
if __name__ == '__main__':
    ...
    #isos delete  add_start_end_depot_info()
    # add_start_time(
    # short_dir='duties-output/short',
    # deadhead_file='static/Deadhead_links_and_running_ times.csv',
    # output_dir='duties-output/Time')
