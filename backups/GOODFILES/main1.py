...
def duties():
    ...
    output3 = pd.DataFrame({
            'VERSION':             "VERSION_U",
            'PROJECT_NAME':        output2['PROJECT_NAME'],
            'PROJUNIT_NAME':       merged2['LINEBASIN'] + "_Daytype",
            'DUTY_COMPANYCODE':    output2['DUTY'],
            'DUTY_ID':             output2['JOURNEY_ID'],
            'PIECETYPE_NAME':      'VB_Piece',
            'REFERREDVB_COMPANYCODE': output2['VB_COMPANYCODE'],
            'PRETIMESEC':          [None] * len(output2),
            'POSTTIMESEC':         [None] * len(output2),
            'STARTNODE_COMPANYCODE': output2['STARTNODE_COMPANYCODE'],
            'STARTNODE_BASIN':       'OSETH',
            'STARTNODE_COMPANY':     'Def',
            'ENDNODE_COMPANYCODE':   output2['ENDNODE_COMPANYCODE'],
            'ENDNODE_BASIN':         'OSETH',
            'ENDNODE_COMPANY':       'Def',
            'START_TIME':            output2['START_TIME'],
            'END_TIME':              output2['END_TIME']
        })
    output3.sort_values(['DUTY_COMPANYCODE','START_TIME'], inplace=True)
    duties_filename = f"{base_name}_Duties_{safe_sheet}.csv"
    duties_path = os.path.join(output_dir, duties_filename)
    output3.to_csv(duties_path, index=False, sep=';', encoding='utf-8')

    print(f" Ολοκληρώθηκε: {filename} | Sheet: {sheet_name} → saved to {output_dir}/")
