    ...
    for file in os.listdir(short_dir):
        ...
        for (vb_code, project_name, projunit_name), group in short_df.groupby(group_keys):
            ...
            if deadhead_matches.empty:
                logging.warning(f"⚠️ Παράλειψη.Δεν υπάρχει {vb_code}  {project_name}  {projunit_name} στο {file}.ενω υπαρχει στο αρχειο deadhead, (δεν βρέθηκε deadhead)")
                continue

            # -- Μία γραμμή --
            if len(group) == 1:
                logging.warning(f"✅ (δεν έχει 2 γραμμές) Μονή εγγραφή για {vb_code} στο {file} — δημιουργείται με βάση ό,τι υπάρχει")
                single_row = group.iloc[0].copy()
                # Προσαρμογή πεδίων (π.χ. start/end ως έχουν, αποθήκευση με dummy τρίτη γραμμή κ.λπ.)
                # Λήψη τιμών από deadhead_matches
                single_row['STARTNODE_COMPANYCODE'] = deadhead_matches['STARTNODE_COMPANYCODE'].iloc[0]
                single_row['ENDNODE_COMPANYCODE'] = deadhead_matches['ENDNODE_COMPANYCODE'].iloc[-1]
                single_row['START_TIME'] = deadhead_matches['START_TIME'].iloc[0]
                single_row['END_TIME'] = deadhead_matches['END_TIME'].iloc[-1]

                final_rows.append(single_row)
                continue
            ...
            # -- Δύο γραμμές --
             ...
             # -- Περίπτωση > 2 γραμμών --
                ...
                logging.warning(f"⚠️ Παράλειψη {vb_code} στο {file} (έχει {len(group)} γραμμές — μη αναμενόμενο)")
            ...
            for (vb_code, project_name, projunit_name), group in result_df.groupby(group_keys):
                ...
                if len(group) == 3:
                    ...
                    if start_node in mapping_dict:
                        ...
                        logging.warning(f"🔍 [3row]Δεν βρέθηκε mapping για START: {start_node} ")

                    if end_node in mapping_dict:
                        result_df.at[group.index[-1], 'ENDNODE_BASIN'] = mapping_dict[end_node]
                    else:
                        logging.warning(f"🔍 [3row]Δεν βρέθηκε mapping για END: {end_node}")
                   ...
                   # Διαγραφή REFERREDVB_COMPANYCODE στη Pre_Duty γραμμή
                    ...
                    # Μονή εγγραφή: mapping μόνο στη μία γραμμή (όπου βρεθεί START ή END)
                    row = group.iloc[0]
                    idx = group.index[0]

                    start_node = row.get('STARTNODE_COMPANYCODE', '')
                    end_node = row.get('ENDNODE_COMPANYCODE', '')

                    if start_node in mapping_dict:
                        result_df.at[idx, 'STARTNODE_BASIN'] = mapping_dict[start_node]
                    else:
                        logging.warning(f"🔍 [1 row] Δεν βρέθηκε mapping για START: {start_node} του{vb_code} {project_name} {projunit_name} ")

                    if end_node in mapping_dict:
                        result_df.at[idx, 'ENDNODE_BASIN'] = mapping_dict[end_node]
                    else:
                        logging.warning(f"🔍 [1 row] Δεν βρέθηκε mapping για END: {end_node}")




            timestamp = datetime.now().strftime('%Y%m%d_%H%M')
            output_path = os.path.join(final_dir, f"final_{timestamp}_{file}")
            #προσθηκη ημερομηνιας στο ονομα αρχειου
